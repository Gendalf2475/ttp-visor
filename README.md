...
`default_stats_format` управляет командой `/stats`: `compact` или `full`. `auto_report_format` управляет автоматическими отчётами и командой `/report`. `compact_top_limit` задаёт размер топа в короткой сводке. Если `compact_show_extra_directions=true`, в коротком отчёте появится блок участников доп. направлений: ТП, КТ и соцсети.
